...
import com.coul.core.domain.base.Repoistory.BaseDaoSpringJdbc;
 ...
 * @author zengshl
 */
public interface EntityDao<E extends Entity> extends BaseDaoSpringJdbc {
	 ...
	 * @author zengshl
	 * 
	 * 案例说明--自己调用自己的操作（一般对组织树，或者存在上下级的调度使用）  前提条件的 上下级关系是1：1的情况下，调用当前上级的数据信息
	 * 前提条件是必须在实体对象里面存在配置关联的关系  reference 和  Association的关联关系   
	 */
	 ...
	 * @author zengshl
	 * 
	 * 案例说明：--前提条件是 被引用的对象是 和 自己本身的对象关系是1：1的情况下，调用该方法实现
	 */
	 ...
	 * 修改说明：
	 * @author zengshl
	 * 案例说明--自己调用自己的操作（一般对组织树，或者存在上下级的调度使用）  前提条件的 上下级关系是1：N的情况下，调用当前上级的数据信息
	 * 
	 */
	...
	/**
	 * 查询该对象关联其他对象的的数据信息，通常有一些1：N的关系，或者N:1的关系需要获取List的数据信息
	 * 
	 * --脚本编写格式如下：select  relationClass.filed  from  relationClass where relationColnum = id;的写法
	 * @param id
	 * @param relationClass
	 * @return
